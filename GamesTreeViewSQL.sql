...
where games.season_year in ( '2015', '2016')
  --and games.season_game_type != 'R'
  and games.season_game_type = 'A'
...
(select distinct(games.season_year), games.season_game_type, refGameType.ref_data_desc gameTypeDesc,
	...
	games.home_team_id home_away_team  --- list under this team id
from retrosheet.dbo.Game_Information games
join retrosheet.dbo.Reference_Data refGameType on games.season_game_type = refGameType.ref_data_code
                                              and refGameType.ref_data_type = 'season_game_type'
join retrosheet.dbo.team homeTeam on games.home_team_id = homeTeam.team_id
...
select distinct(games.season_year), games.season_game_type, refGameType.ref_data_desc gameTypeDesc,
	...
	games.game_id, 
	homeTeam.league home_away_league,
	games.visiting_team_id home_away_team  --- list under this team id
from retrosheet.dbo.Game_Information games
join retrosheet.dbo.Reference_Data refGameType on games.season_game_type = refGameType.ref_data_code
                                              and refGameType.ref_data_type = 'season_game_type'
join retrosheet.dbo.team homeTeam on games.home_team_id = homeTeam.team_id
...
---- ***************************
---- all games for game selection TreeView
select * from
(---- Post Season and All Star games
select distinct(games.season_year), games.season_game_type, refGameType.ref_data_desc gameTypeDesc,
	games.game_date,
...
select distinct(games.season_year), games.season_game_type, refGameType.ref_data_desc gameTypeDesc,
	...
	visitTeam.league, refVisitTeamLeague.ref_data_desc, games.visiting_team_id, visitTeam.name, visitTeam.city,
	games.game_id, 
	homeTeam.league display_under_league,
	games.home_team_id display_under_team,  --- list under this team id
	case games.season_game_type
		when 'R' then 0
		when 'A' then 1
		when 'C' then 2
...
select distinct(games.season_year), games.season_game_type, refGameType.ref_data_desc gameTypeDesc,
	...
	games.game_id, 
	homeTeam.league display_under_league,
	games.visiting_team_id display_under_team,  --- list under this team id
	case games.season_game_type
		when 'R' then 0
		when 'A' then 1
		when 'C' then 2
...
---where games.season_year = '2015'
  where games.season_game_type = 'R') 
										unionTable
order by season_year, sort_key, display_under_league, display_under_team, game_date, game_id;